    ...
    <el-table
      ...
      <el-table-column label="操作" align="center" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button type="danger" icon="el-icon-delete" size="mini" @click="removeSysUserById(scope.row.id)"
                     title="删除"/>
  ...
  methods: {
    ...
    removeSysUserById(id) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        return api.removeSysUserById(id)
      }).then((response) => {
    ...
    switchStatus(row) {
      ...
      api.switchStatus(row.id, row.status).then(response => {
        this.$message.success(response.message || '操作成功')
        this.fetchData(this.page)
      })
    },
